...
describe('useFrequencyData', () => {
  ...
  it('should return empty array when not initialized', () => {
    ...
    const { result } = renderHook(() => useFrequencyData(analyser as unknown as AnalyserNode, false));
    const getFrequencyData = result.current;
  ...
  it('should return frequency data when initialized with analyzer', () => {
    const analyser = new MockAnalyserNode();
    const { result } = renderHook(() => useFrequencyData(analyser as unknown as AnalyserNode, true));
    const getFrequencyData = result.current;
  ...
  it('should update data array when analyzer changes', () => {
    ...
    const { result, rerender } = renderHook(
      ({ analyser, isInitialized }) => useFrequencyData(analyser as unknown as AnalyserNode, isInitialized),
      {
